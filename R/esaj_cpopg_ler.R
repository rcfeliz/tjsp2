...
#' esaj_cpopg_ler() parseia os dados de um html do cpopg.
#'
...
#' @param formato Formato das colunas a serem retornadas. É escolhido 'Padronizado' por default.
...
#'
#' @export
esaj_cpopg_ler <- function(arquivos = NULL,
...
esaj_cpopg_ler_unitario <- function(arquivo = NULL,
                                    formato = c("Padronizado", "Completo"),
                                    outros = c("Tudo", "Delegacia", "Partes", "Movimentacoes", "Peticoes Diversas", "Incidentes", "Apensos", "Audiencias", "Historico Classes")) {

  # extrai informacoes de capa
  if(length(formato) > 1) {
    formato <- "Padronizado"
  ...
  if(formato == "Padronizado") {
    aux_capa <- esaj_cpopg_ler_capa_padronizado(arquivo)
  } else {
    aux_capa <- esaj_cpopg_ler_capa_completo(arquivo)
  }
  ...
  if(is.null(outros)) {
    ...
    aux_outros <- purrr::map(outros, ~{
      fn_nm <- glue::glue("esaj_cpopg_ler_{.x}")
      f <- get(fn_nm)
...
esaj_cpopg_ler_capa_padronizado <- function(arquivo = NULL) {
  html <- arquivo |>
    xml2::read_html()
  ...
  # Informacoes que mudam a forma de extrair com base no tipo de processo
  ...
  # Informacoes que se extraem da mesma forma, independente do tipo de processo
  digital <- html |>
...
esaj_cpopg_ler_capa_completo <- function(arquivo = NULL) {
...
esaj_cpopg_ler_partes <- function(arquivo = NULL) {
...
esaj_cpopg_ler_movimentacoes <- function (arquivo = NULL) {
...
esaj_cpopg_ler_delegacia <- function(arquivo = NULL) {
...
esaj_cpopg_ler_peticoes_diversas <- function(arquivo = NULL) {
  html <- arquivo |>
    xml2::read_html()
...
esaj_cpopg_ler_incidentes <- function(arquivo = NULL) {
  html <- arquivo |>
    xml2::read_html()
...
esaj_cpopg_ler_apensos <- function(arquivo = NULL) {
  html <- arquivo |>
    xml2::read_html()
...
esaj_cpopg_ler_audiencias <- function(arquivo = NULL) {
  html <- arquivo |>
    xml2::read_html()
...
esaj_cpopg_ler_historico_classes <- function(arquivo = NULL) {
  html <- arquivo |>
    xml2::read_html()
