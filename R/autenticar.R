...
tjsp_autenticar <- function(login = NULL, password = NULL,
  ...
  get_email_token_insistente <- purrr::insistently(get_email_token, taxa, quiet = FALSE)
...
#' Verifica se o login no site do TJSP está ativo
#'
#' Esta função realiza uma requisição ao endpoint de verificação de login do site
#' do Tribunal de Justiça de São Paulo (TJSP) e verifica se a sessão associada aos
#' cookies fornecidos está autenticada.
#'
#' @param cookies Um objeto de cookies (geralmente do tipo `httr2::cookie`) que representa
#' os dados de sessão autenticada previamente salvos.
#'
#' @return Um valor lógico (`TRUE` ou `FALSE`). Retorna `TRUE` se o login estiver válido (sessão ativa)
...
#' @returns token
#'
get_email_token <- function(email_provider, outlook, hora) {
