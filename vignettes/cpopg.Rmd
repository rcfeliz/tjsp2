...
tjsp_cpopg_baixar_outros(parametro = parametro, consultas = consultas)
```
...
Cada página do esaj contém no máximo 25 resultados. Assim, se uma consulta gerar mais de 25 resultados, então haverá mais de uma página para aquela consulta. Imaginemos, por exemplo, que o nome "Maria do Carmo" gerou 30 resultados. Então a consulta `tjsp_cpopg_baixar_outros(parametro = "NMPARTE", consultas = "Maria do Carmo")` iria gerar os seguintes arquivos em html:
...
A função `tjsp_cpopg_ler_outros()` recebe o arquivo em html e retorna uma tibble contendo as seguintes colunas:
...
## `tjsp_cpopg_*_tabela()` e `tjsp_cpopg_*documentos()`
